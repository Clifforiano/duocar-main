...
obtenerIdConductor(viajeId: string): Observable<string> {
  ...
  // Método para obtener el estado de reserva de un usuario
  getEstadoReserva(uid: string): Observable<boolean> {
    return this.firestore.collection('users').doc(uid).valueChanges().pipe(
      map((user: User | undefined) => user?.estado_reserva || false)
    );
  }

  // Método para obtener el estado de conductor de un usuario dado su UID
  getEstadoConductor(uid: string): Observable<boolean> {
    return this.firestore.collection('users').doc(uid).valueChanges().pipe(
      map((user: User | undefined) => user?.estado_conductor || false)
    );
  }
