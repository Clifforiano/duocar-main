 ...
 async ionViewWillEnter() {
  ...
  await Network.getStatus().then(status => {
    ...
    this.viajeService.getEstadoViaje(viajeId).subscribe(
                  ...
                  if (this.cachedUsuarios[id]) {
                    ...
                    this.viajeService.getUsuarioPorId(id).subscribe(pasajero => {
                      if (pasajero) {
                        ...
                        localStorage.setItem('pasajeros_datos', JSON.stringify(this.pasajeros));
                        loading.dismiss();
      ...
      error => {
        ...
        this.viajeService.obtenerReservas(this.viajeid).subscribe(reservas => {
          this.reservas = reservas
            localStorage.setItem('valores_reservas',reservas.toString());
      ...
      this.reservas= localStorage.getItem('valores_reservas')

      this.pasajeros = JSON.parse(localStorage.getItem('pasajeros_datos') || '[]');
      loading.dismiss();
  ...
  async cancelarViaje() {
    ...
    this.utilsSvc.presentToast({
      message: 'Viaje cancelado con éxito de manera exitosa.',
      color: 'success',
      position: 'middle',
      duration: 2000,
